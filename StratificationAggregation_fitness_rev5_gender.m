function [speciesNames,y_ctrl,y_perturb,y_full,y_out] = StratificationAggregation_fitness_rev5_gender(w,w_map,w_full,exptin,exptout,inputs_idx,gender_value,perturb_nodes,perturb_lvl) 
...
load(exptin,'input_scaled');
load(exptout,'output_scaled');
isFemale = contains(input_scaled.Properties.RowNames, ["TAVRA","TAVRB","TAVRC","TAVRD"]);
...
outputs_idx = zeros(1,length(output_scaled.Properties.VariableNames));
for output = 1:length(outputs_idx)
    idx = find(strcmpi(output_scaled.Properties.VariableNames(output),speciesNames));
    outputs_idx(output) = idx;
end

% initialize storage arrays
y_ctrl = zeros(length(perturb_nodes), length(speciesNames), size(input_scaled, 1));
y_perturb = zeros(length(perturb_nodes), length(speciesNames), size(input_scaled, 1));
y_full= zeros(length(perturb_nodes), length(speciesNames), size(input_scaled, 1));

% run simulations: loop through all conditions
for patient = 1:size(input_scaled,1)
    w_stim = w_expand;                  % reset w/ each patient
    
    % modify input rxns
    for species = 1:length(inputs_idx)
        w_stim(inputs_idx(species)) = input_scaled{patient, species};
    end
    ...
    % run control simulation for patient
    rpar_stim = [w_stim; n; EC50];        % integrate w_stim into params
    params_stim = {rpar_stim, tau, ymax, speciesNames};
    tspan = [0 80];
    [~,y1] = ode23(@ODEfun, tspan, y0, options, params_stim);
    
    % loop through all perturbations for patient
    for perturbnode = 1:length(perturb_nodes)
        ymax_perturb = ymax;            % ymax_perturb: set to ymax, then specific values modified
        ymax_perturb(perturb_nodes(perturbnode)) = perturb_lvl * ymax_perturb(perturb_nodes(perturbnode));
        params_perturb = {rpar_stim, tau, ymax_perturb, speciesNames};
        tspan = [80 320];
        [~,y2] = ode23(@ODEfun, tspan, y1(end,:), options, params_perturb);
        
        % store steady-state values
        y_ctrl(perturbnode, :, patient) = real(y1(end,:));
        y_perturb(perturbnode, :, patient) = real(y2(end,:));
    end
    
    % store delta(Activity) levels for perturbations
    y_full(:, :, patient) = y_perturb(:, :, patient) - y_ctrl(:, :, patient);
end

% filter for output nodes
y_out = y_full(:, outputs_idx, :);
