function [speciesNames,y_default,y_perturb,y_sens,ymax_sens] = EnsembleSensitivity_rev5_gender(w,w_map,w_full,exptin,exptout,inputs_idx,gender_value) 
...
% initialize storage arrays for sensitivity
y_default = zeros(length(speciesNames) ,length(speciesNames), size(input_scaled, 1));
y_perturb = zeros(length(speciesNames), length(speciesNames), size(input_scaled, 1));
ymax_sens = zeros(length(speciesNames), length(speciesNames), size(input_scaled, 1));
...
for patient = 1:size(input_scaled, 1)
    ...
    % run default simulation (no knockdown)
    ...
    % loop through all node knockdowns
    for perturb = 1:length(speciesNames)
        ymax_perturb = ymax;
        ymax_perturb(perturb) = 0.1 * ymax_perturb(perturb);
        ...
        y_default(perturb, :, patient) = real(y1(end,:));
        y_perturb(perturb, :, patient) = real(y2(end,:));
        ymax_sens(perturb, :, patient) = ymax_perturb;
    end
    
end

% calculate sensitivity metric: delta(Activity)
y_sens = (y_perturb - y_default);
