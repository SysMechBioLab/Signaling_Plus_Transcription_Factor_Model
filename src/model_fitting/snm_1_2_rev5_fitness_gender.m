function error = snm_1_2_rev5_fitness_gender(w,w_map,w_full,exptin,exptout,inputs_idx,gender_value) 
...
% expand reduced param set to full system
...
for stimulus = 1:size(input_scaled,1)
    % set w values for each patient
    w_stim = w_expand;
    for species = 1:length(inputs_idx)
        w_stim(inputs_idx(species)) = input_scaled{stimulus,species};
    end
...
% SE = ((y_delta - expt_delta).^2) ./ ((expt_delta - mean(expt_delta,"all")).^2);         % normalized by deviance from mean
MSE = mean(SE,"all");                   % average across all elements
