function [speciesNames,y_perturb] = StratificationSimulation_doseresponse(w,w_map,w_full,exptin,exptout,inputs_idx,gender_value,perturb_nodes,perturb_lvls) 
...
% load(exptin,'input_scaled');          % TAVR_A-H (as array)
load(exptin,'input_scaled_protonly');   % TAVR_I-L (as table)
input_scaled = input_scaled_protonly;   % coerce to array
...
y_perturb = zeros(length(perturb_nodes), ...
                  length(speciesNames), ...
                  length(perturb_lvls)+1, ...
                  size(input_scaled, 1));
...
for patient = 1:size(input_scaled,1)
    ...
    for perturbnode = 1:length(perturb_nodes)
        y_perturb(perturbnode, :, 1, patient) = real(y1(end,:));                   % y(:,:,1,stimulus) = ctrl
        
        for perturblvl = 1:length(perturb_lvls)
            ymax_perturb = ymax;            % reset w/ each dose
            ymax_perturb(perturb_nodes(perturbnode)) = perturb_lvls(perturblvl) * ymax_perturb(perturb_nodes(perturbnode));
            ...
            y_perturb(perturbnode, :, perturblvl+1, patient) = real(y2(end,:));    % y(:,:,2_end,stimulus) = perturbs
        end
    end   
end
