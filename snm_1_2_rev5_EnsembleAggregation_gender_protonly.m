...
exptin = 'testingdata_inputs_Anseth_12312020.mat';
...
y_out_all_protonly = zeros(8,23,size(x_all,1));
y_full_all_protonly = zeros(8,151,size(x_all,1));
y_delta_all_protonly = zeros(4,23,size(x_all,1));
y_delta_full_all_protonly = zeros(4,151,size(x_all,1));
...
parfor run = 1:size(x_all, 1)
    ...
    [~,y_out,y_full,y_delta,y_delta_full] = EnsembleAggregation_fitness_rev5_gender_protonly(w, ...
                                                                                            w_map, ...
                                                                                            w_full, ...
                                                                                            exptin, ...
                                                                                            exptout, ...
    ...
    y_out_all_protonly(:,:,run) = y_out;
    y_full_all_protonly(:,:,run) = y_full;
    y_delta_all_protonly(:,:,run) = y_delta;
    y_delta_full_all_protonly(:,:,run) = y_delta_full;
    % SE_all(:,:,run) = SE;
...
[speciesNames,y_out_nofit,y_full_nofit,y_delta_nofit,y_delta_full_nofit] = EnsembleAggregation_fitness_rev5_gender_protonly(w_nofit, ...
                                                                                                                            w_map, ...
                                                                                                                            w_full, ...
                                                                                                                            exptin, ...
                                                                                                                            exptout, ...
...
save("ga_aggregate_results_protonly.mat", "y_out_all_protonly", "y_full_all_protonly", "y_delta_all_protonly", "y_delta_full_all_protonly");
save("ga_aggregate_nofit_protonly.mat", "y_out_nofit", "y_full_nofit", "y_delta_nofit", "y_delta_full_nofit");
